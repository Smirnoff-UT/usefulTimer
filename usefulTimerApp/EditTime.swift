...
func setupEditView( _self : UIViewController, page : Int){
    ...
    // サイズを指定する.
    hourPicker.frame = CGRectMake(_self.view.bounds.width/4 ,_self.view.bounds.height/10, _self.view.bounds.width/2, 400.0)
    minutePicker.frame = CGRectMake(0,_self.view.bounds.height/4, _self.view.bounds.width/10, 180.0)
    secondPicker.frame = CGRectMake(0,_self.view.bounds.height/4, _self.view.bounds.width/10, 180.0)
    ...
    cancelButton.addTarget(_self, action: "tapCancelButton:", forControlEvents: .TouchUpInside)
